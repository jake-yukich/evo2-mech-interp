...
def get_mean_embeddings(
    ...
    if not force_recompute and all_embeddings_file.exists():
        ...
        if mean_embeddings_tensor.shape[1] != d_model:
            print(
                f"Warning: Cached embeddings have wrong dimension ({mean_embeddings_tensor.shape[1]} != {d_model})"
            )
            print("Recomputing embeddings...")
        elif mean_embeddings_tensor.shape[0] != len(df):
            print(
                f"Warning: Cached embeddings have wrong number of genomes ({mean_embeddings_tensor.shape[0]} != {len(df)})"
            )
            print("Recomputing embeddings...")
        else:
            print(
                f"Successfully loaded {mean_embeddings_tensor.shape[0]} genome embeddings from cache"
            )
    ...
    for genome_idx, tokenized_samples_from_genome in enumerate(
        ...
        if not force_recompute and genome_cache_file.exists():
            try:
                ...
                if genome_embedding.shape == (d_model,):
                    ...
                    print(
                        f"Warning: Cached genome {genome_idx} has wrong shape, recomputing..."
                    )
            except Exception as e:
                print(
                    f"Warning: Error loading cached genome {genome_idx}: {e}, recomputing..."
                )

        # Compute embedding for this genome
        sample_embeddings = batch_inference(
            tokenized_samples_from_genome,
            evo2_model,
            batch_size,
            region_length,
            d_model,
        ...
        assert genome_embedding.shape == (d_model,), (
            f"Expected shape ({d_model},), got {genome_embedding.shape}"
        )
    ...
    print(
        f"Saved {mean_embeddings.shape[0]} genome embeddings to {all_embeddings_file}"
    )
