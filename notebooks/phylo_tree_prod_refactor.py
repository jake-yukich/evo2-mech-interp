...
print(f"Sampled {len(samples_df)} regions from {len(samples_df['genome_idx'].unique())} genomes")
samples_df.head()
...
print(f"Loaded {config.model_name} with {sum(p.numel() for p in evo2_model.model.parameters() if p.requires_grad):,} parameters")
